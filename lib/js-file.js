...
var JsFile = function(filename, source, tree) {
    ...
    // Inline config is malformed json, so this avoids a messy regex
    ...
    var program = this.getNodesByType('Program')[0];
    if (program.comments) {
        program.comments.forEach(function(comment) {
            if (comment.value.indexOf('jscs') !== -1) {
                this._config = parseConfigString(comment.value);
            }
        }, this);
    }
};
