...
var generateConfig = require('./config/generate');
...
module.exports = function(program) {
     ...
     */
    if (!config && !program.preset && !program.configure) {
        if (program.config) {
    ...
    if (!args.length && process.stdin.isTTY && !program.configure) {
    ...
    if (program.configure) {
        generateConfig({
            checker: checker,
            path: program.configure
        })
        .then(function(config) {
            fs.writeFileSync(process.cwd() + '/.jscsrc', JSON.stringify(config, null, '\t'));
            defer.resolve(0);
        }, function() {
